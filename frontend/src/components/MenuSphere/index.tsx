...
export function MenuSphere() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Center ball pulses on a sine wave with its own phase.
      ...
      if (!panelOpen) {
        const centerPulse = Math.sin(t * 1.2 + center.pulsePhase);
        const centerScale = 1 + centerPulse * 0.08;
        center.group.scale.setScalar(centerScale);
        center.haloMat.opacity =
          0.12 + (centerPulse * 0.5 + 0.5) * 0.12;
      }

      // Per-satellite pulse + hover feedback + depth fade.
      // When a panel is open we hide the whole menu — the user is
      // focused on the panel, so the balls would just be visual
      // noise underneath it. ``panelOpen`` was computed above for
      // the center ball; reuse it here.
      ...
      for (const s of satellites) {
        ...
        const depth01 = Math.max(0, Math.min(1, (camPos.z + 11) / 6));

        const pulse01 = Math.sin(t * s.pulseSpeed + s.pulsePhase) * 0.5 + 0.5;
        const hoverBoost = s.hovered ? 0.25 : 0;
        const depthMul = 0.1 + depth01 * 0.9;

        s.haloMat.opacity =
          (0.45 + pulse01 * 0.35 + hoverBoost) * depthMul;
        s.shellMat.opacity = 0.95 * depthMul;
        s.coreMat.opacity = 1.0 * depthMul;

        const pulseScale = 1 + pulse01 * 0.12;
        const hoverTarget = s.hovered ? 1.5 : 1.0;
        s.group.scale.setScalar(pulseScale * hoverTarget);

        s.lineMat.opacity =
          ((s.hovered ? 0.95 : 0.35) + pulse01 * 0.15) * depthMul;
        s.labelEl.style.opacity = String(
          (s.hovered ? 1.0 : 0.95) * (0.15 + depth01 * 0.85)
        );
      }
